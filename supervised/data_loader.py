...
class DataLoader():
    def __init__(self, morphology, lower_case):
        ...
        if self.lower_case:
            self.characters = sorted(list(morphology["LOWER_CHARACTERS"]))
        else:
            self.characters = sorted(list(morphology["CHARACTERS"]))
        self.characters.extend(FUNCTIONAL_TOKENS)
        self.all_feats = []
        for feat in self.morphology["VAL2ATTR"].keys():
            self.all_feats.append(feat)
        self.all_feats = sorted(self.all_feats)
        self.disallowed_feats = morphology["DISALLOWED_FEATS"]
    ...
    def tags_to_tensors(self, tags):
        ...
        for bi, tag in enumerate(tags):
            ...
            feat_lists = []
            for feat in feats:
                feat_vals = [fv for fv in feat.split(".") if fv not in self.disallowed_feats]
                if feat_vals:
    ...
    def sample_batch(self, keys, freq_dist, lines, lemma_to_lines, size, mode="INFLECT"):
